...
//			button3.Enabled = false;
		...
		private void button1_Click(object sender, System.EventArgs e)
		{
			...
			//IPHostEntry ipHostInfo = Dns.Resolve(Dns.GetHostByName(textBox1.Text).ToString());
			IPAddress ipAddress = ipHostInfo.AddressList[0];
			IPEndPoint localEndPoint = new IPEndPoint(ipAddress,Convert.ToInt16(textBox2.Text));
...
//			button3.Enabled = true;
			...
			serverHandler = serverListener.Accept();
			label3.Text = "CONNECTION ACCEPTED FROM " + serverHandler.RemoteEndPoint.ToString().ToUpper() + " AND ESTABLISHED.";
		...
		{
			serverHandler.Shutdown(SocketShutdown.Both);
			serverHandler.Close();
			//serverHandler.Close();
//			button1.Enabled = true;
//			button3.Enabled = false;
			label3.Text = "CONNECTION STOPED.";
		}
		...
		{
			if (serverHandler.Available != 0)
			{
				incomingMsgLength = serverHandler.Receive(incomingMsg);
